...
impl BgfxRenderer {
    ...
    pub fn new(
        ...
        Self {
            ...
            shaders: HashMap::new(),
            default_clear_color: default_color,
        }
    }
}
            ...
            match object.get_type() {
                ObjectTypes::Colored => {
                    ...
                    let vertex_buffer = unsafe {
                        let shaders = colored.shaders.borrow();

                        let bgfx_shaders: &BgfxShaderContainer = shaders
                            .as_any()
                            .downcast_ref()
                            .expect("Invalid shader type");

                        let layout = bgfx_shaders.get_layout_builder();

                        let vertices_reference = colored.vertices.as_ref();

                        let memory = Memory::reference(vertices_reference);
                        bgfx::create_vertex_buffer(&memory, layout, BufferFlags::empty().bits())
                    };

                    let index_buffer = unsafe {
                        let index_ref = colored.indices.as_ref();
                        let memory = Memory::reference(index_ref);
