...
use event_bus::{dispatch_event, subscribe_event, Event, EventResult};
use glam::Vec3;
...
impl SceneManager {
    pub fn new() -> Self {
        ...
        scene_map.insert(
            String::from(&default_scene.name.clone()),
            Rc::new(RefCell::new(default_scene)),
        );
...
mod tests {
    ...
    use event_bus::{subscribe_event, Event, EventBus};
    ...
    fn render_scene_test() {
        ...
        unsafe {
            set_renderer(RendererSim::new(
                mamager.get_scene(String::from("test")).unwrap(),
            ))
        }
        ...
        mamager
            .get_scene(String::from("test"))
            .unwrap()
            .borrow_mut()
            .add_chunk(chunk, Vec2::new(-5.0, -5.0), Vec2::new(5.0, 5.0));
